...
def generate_image_name(filename: str) -> str:
    return filename.replace(".log", ".png")


def main(argv: list[str]):
    target_log_file = argv[0]

    if not re.match(r"cpu_temp_\d+.log", target_log_file):
        print("Invalid file name, must match 'cpu_temp_{int}.log")
        exit(1)

    image_file = (
        "out/" + generate_image_name(target_log_file)
        if not target_log_file.startswith("data/")
        else generate_image_name(target_log_file)
    )
